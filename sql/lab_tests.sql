...
, lab_tests_cohort as (
    select co.hadm_id, lab.charttime, lab.label, lab.valuenum,
           case when lab.label = 'wbc' and lab.valuenum between 4.5 and 10 then 0
                when lab.label = 'hgb' and co.gender = 'M' and lab.valuenum between 13.8 and 17.2 then 0
                when lab.label = 'hgb' and co.gender = 'F' and lab.valuenum between 12.1 and 15.1 then 0
                when lab.label = 'platelet' and lab.valuenum between 150 and 400 then 0
                when lab.label = 'sodium' and lab.valuenum between 135 and 145 then 0
                when lab.label = 'potassium' and lab.valuenum between 3.7 and 5.2 then 0
                when lab.label = 'bicarbonate' and lab.valuenum between 22 and 28 then 0
                when lab.label = 'chloride' and lab.valuenum between 96 and 106 then 0
                when lab.label = 'bun' and lab.valuenum between 6 and 20 then 0
                when lab.label = 'lactate' and lab.valuenum between 0.5 and 2.2 then 0
                when lab.label = 'creatinine' and co.gender = 'M' and lab.valuenum <= 1.3 then 0
                when lab.label = 'creatinine' and co.gender = 'F' and lab.valuenum <= 1.1 then 0
                when lab.label = 'ph' and lab.valuenum between 7.38 and 7.42 then 0
                when lab.label = 'po2' and lab.valuenum between 75 and 100 then 0
                when lab.label = 'pco2' and lab.valuenum between 35 and 45 then 0
                when lab.label = 'bnp' and lab.valuenum <= 100 then 0
                when lab.label = 'troponin' and lab.valuenum <= 0.1 then 0
                when lab.label = 'creatinine_kinase' and lab.valuenum <= 120 then 0
                else 1 end as abnormal
    from basics co
    left join lab_tests lab using (hadm_id)
    where lab.charttime between co.intime and co.intime + interval '1 day'
          and lab.charttime between co.intime and co.outtime
          and lab.label is not null
